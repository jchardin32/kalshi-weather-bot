# “””
PCV1 aka V26 – Kalshi Weather Bot

Incorporates v24 + v24.1 + v24.2 review fixes plus V26 deploy fixes.

V26 ADJUSTMENTS
Z1. Private key loadable from KALSHI_PRIVATE_KEY_PEM env var (preferred for
cloud deploys like Render). Falls back to KALSHI_PRIVATE_KEY_PATH for
local dev. Supports raw PEM, escaped “\n” PEM, or base64-encoded PEM.
...
GFS (31) have different intrinsic spreads; full-pool sigma over-weights the
larger ensemble.
...
1. is_liquid: max-spread gate, scales with price.
1. _to_cents: symmetric ROUND_HALF_EVEN for both asks and bids (v24.1).
1. precipitation_prob: explicit length_unit=metric to lock cm/mm contract.
1. LOW pace adjustment: only applied to overnight METAR window (v24.1).
1. METAR cache key: hourly precision, no target_date pollution (v24.1).
1. Position recorded only after fill confirmation.
1. RequestBudget: hard write-reserve floor.
...
- Forecast disagreement skip (ECMWF vs GFS > 1.5sigma) for both HIGH and LOW.
...
GFS_PREFIX = “gfs025”
DISAGREEMENT_SIGMA_LIMIT = 1.5      # skip if model means disagree > 1.5sigma
...
“””
Load the Kalshi private key PEM, in priority order:
1. KALSHI_PRIVATE_KEY_PEM env var (preferred for cloud deploys like
Render – no filesystem dependency, key lives in the platform’s
secret store). Supports raw PEM, escaped “\n” PEM, or base64-
encoded PEM in case the platform mangles newlines.
2. KALSHI_PRIVATE_KEY_PATH env var (for local dev – points at .pem file).

```
Render and similar PaaS hosts don't have persistent disk for committed
files, and committing the key to git is bad practice. Use the env-var
path in production.
"""
pem = PRIVATE_KEY_PEM
if pem:
    # Some UIs strip real newlines and store literal "\n"
    if "\\n" in pem and "\n" not in pem:
        pem = pem.replace("\\n", "\n")
    # Or accept base64-encoded PEM:
    #   base64 -i your_kalshi_key.pem | tr -d '\n' | pbcopy
    if "BEGIN" not in pem:
        try:
            import base64
            ...
            log.error(
                "KALSHI_PRIVATE_KEY_PEM is set but not a valid PEM "
                "or base64-encoded PEM"
            )
            sys.exit(1)
    if "BEGIN" not in pem:
        log.error("KALSHI_PRIVATE_KEY_PEM does not contain a PEM block")
        sys.exit(1)
...
log.error(
    "No Kalshi private key available. Set KALSHI_PRIVATE_KEY_PEM "
    "(production) or KALSHI_PRIVATE_KEY_PATH (local dev)."
)
sys.exit(1)
```
...
if is_paper:
    ...
    log.info("[%s] %s %s %s %.0f @ %dc shrunk_edge=%+.1fc",
...
if filled and taken > 0:
    ...
    msg = (f"FILL [{intent['profile']}] {intent['ticker']} "
           ...
           f"{base_position.entry_price_cents}c "
