...
MASTER_CITY_COORDS = { ... }  # (your full list from previous version - keep it)

_forecast_cache = {}
_seen_edges = {}

# (send_telegram, parse_ticker, minutes_to_expiry, _normal_cdf, get_model_prob, cached_model_prob, log_to_csv, get_current_bankroll, place_order, fetch_weather_markets stay exactly as in v4)

print("🚀 DEBUG v5 - RAW TICKER + PARSED logging - April 27 2026 2:10 PM CDT")
...
while True:
    try:
        ...
        for m in markets:
            try:
                ticker = m.get("ticker", "")
                log.info("RAW TICKER: %s", ticker)   # ← this should always appear for weather markets
                ...
                # (rest of the loop stays the same as v4)
